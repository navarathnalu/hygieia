...
const help = ctx => {
  const features = ['Hygiea can show estimated next period date', 'notify you about your period', 'show statistics about your periods\\.',];
  const featuresMsg = features.join(', ');
  const start = 'Without any late, start tracking your periods with Hygieia by sending _/track_ command\\.';
...
const replyWithStatus = (ctx, user) => {
  ...
  ctx.replyWithMarkdownV2(`*Cycle Details*\n\nNext period on: ${nextDate.format(constants.date_format)}\nLast period date: ${lastDate.format(constants.date_format)}\nCycle length: ${user.cycleLength}`);
};
